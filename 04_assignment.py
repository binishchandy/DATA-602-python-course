...
class PandasChain:
    ...
    def add_transaction(self, s, r, v):
        # self.__all_txn_values.append(v)
    ...
    def __commit_block(self, block):
        ...
        block_hash_concat = ''
        if self.__prev_hash:
            block_hash_concat += self.__prev_hash
        block_hash_concat += self.__id
        block_hash_concat += str(dt.datetime.now())
        block_hash_concat += str(self.__seq_id)
        block_hash_concat += self.__current_block.get_simple_merkle_root()
        block_hash = hashlib.sha256(block_hash_concat.encode('utf-8')).hexdigest()  # Create block hash
    ...
    def get_values(self):
        ...
        all_txn_ts = []
        for blk in self.__chain:
            (value, ts) = blk.get_values()
            all_txn_values.extend(value)
            all_txn_ts.extend(ts)
        (value, ts) = self.__current_block.get_values()
        all_txn_values.extend(value)
        all_txn_ts.extend(ts)
        return all_txn_values
#     For extra credit comment out return statement above and include statement below
#         return all_txn_ts
...
class Block:
    ...
    def display_header(self):
        header = "Seq Id : {seq_id}, Status : {status}, Block Hash : {block_hash}, " \
                 "Merkle Hash : {merkle_hash}, Number of Txn : {tx_count}" \
            .format(seq_id=self.__seq_id,
                    status=self.__status,
                    block_hash=self.__block_hash,
                    merkle_hash=self.__merkle_tx_hash,
                    tx_count=self.__transactions.shape[0])
        print(header)
    ...
    def add_transaction(self, s, r, v):
        ...
        self.__txn_values.append(v)
        self.__txn_timestamps.append(ts)
        tx_hash = hashlib.sha256(
            str(str(ts) + s + r + str(v)).encode('utf-8')).hexdigest()  # Hash of timestamp, sender, receiver, value
        new_transaction = pd.DataFrame([[ts, s, r, v, tx_hash]],
                                       columns=self.__col_names)  # Create DataFrame with transaction data (a DataFrame with only 1 row)
    ...
    def set_status(self, status):
        if status in ['COMMITTED', 'UNCOMMITTED']:
    ...
    def get_simple_merkle_root(self):
        ...
        for index, row in self.__transactions.iterrows():
            hash_concat += str(row['TxHash'])
        self.__merkle_tx_hash = hashlib.sha256(hash_concat.encode('utf-8')).hexdigest()
        return self.__merkle_tx_hash

    def get_values(self):
       return (self.__txn_values, self.__txn_timestamps)
...
class TestAssignment4(unittest.TestCase):
    def test_chain(self):
        ...
        #to test extra credit question, comment out below code
        # x = np.array(pandas_chain.get_values())
