...
def exercise01():
    ...
    # ------ Place code below here \/ \/ \/ ------
    df = pd.DataFrame(np.random.randint(low=0, high=10, size=(3, 4)))
...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    #np.array([[(i + j) % 2 for i in range(5)] for j in range(4)])
...
def exercise13(n):
    ...
    for i in range(n):
        index_array.append(
            pd.Timestamp((start_date + datetime.timedelta(days=i, hours=0, minutes=0, seconds=0)).strftime('%Y-%m-%d %H:%M:%S')))
        rand_int_array.append(random.randint(0, n))

    s = pd.Series(data=rand_int_array, index=index_array)
    s.cumsum().plot.line()
    plt.show()
...
def exercise15():
    ...
    # ------ Place code below here \/ \/ \/ ------
    url = "http://samplecsvs.s3.amazonaws.com/Sacramentorealestatetransactions.csv"
    df = pd.read_csv(url, skiprows=lambda x: x % 5 == 0)
