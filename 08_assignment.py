...
class Iris:
    ...
    def __create(self):
        ...
        drop_db = "DROP DATABASE {}".format(self.__dbname)
        create_db = "CREATE DATABASE {}".format(self.__dbname)
        use_db = "USE {}".format(self.__dbname)
        drop_table = "DROP TABLE IF EXISTS iris_data"
        create_table = '''
                                            CREATE TABLE iris_data (
                	                        id INT NOT NULL,
                                            feature_sepal_length FLOAT NOT NULL,
    ...
    def load(self,truncate=False):
        ...
        add_iris_row = ("INSERT INTO iris_data "
        ...
        for i, row in df.iterrows():
            row_data = (i,
                        float(row['sepal length (cm)']),
                        float(row['sepal width (cm)']),
                        float(row['petal length (cm)']),
                        float(row['petal width (cm)']),
                        str(target_names[int(target[i])]),
                        int(target[i]))
            self.__cursor.execute(add_iris_row, row_data)
        self.__conn.commit()
        # ------ Place code above here /\ /\ /\ ------
    ...
    def display_gt(self,n): 
        # ------ Place code below here \/ \/ \/ ------
        select_query = (
            "select id, feature_sepal_length, feature_sepal_width, feature_petal_length, feature_petal_width, target_species, target_species_id from iris_data "
            "where id > %s")

        self.__cursor.execute(select_query, (n,))
    ...
    def update_observation(self,id,new_target_species,new_target_species_id):
        # ------ Place code below here \/ \/ \/ ------
        update_query = ("update iris_data "
                        "set target_species = %s, target_species_id = %s")
        self.__cursor.execute(update_query, (new_target_species, new_target_species_id))
        self.__conn.commit()
    ...
    def del_observations(self,row_ids):
        # ------ Place code below here \/ \/ \/ ------
        delete_query = ("delete from iris_data "
                        "where id = %s")
        for row_id in row_ids:
            self.__cursor.execute(delete_query, (row_id,))
        self.__conn.commit()
    ...
    def get_row_count(self):
        # ------ Place code below here \/ \/ \/ ------
        count_query = "select count(*) from iris_data"
